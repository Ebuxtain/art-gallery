  ...
  try {
    ...
    auctions.forEach(async (auction: any) => {
      ...
      if (new Date(auction.startDate).getTime() >= new Date().getTime()) {
        ...
        status = Status.LIVE;
        await sendNotification(`The excitement is happening now! 🎉 Our live auction event is currently underway, and you're invited to join the bidding frenzy. Don't miss your chance to snag incredible artworks and immerse yourself in the thrill of the auction.`, RecipientType.USER);
      } else {
        status = Status.ENDED;
        await sendNotification(`The curtains have closed on our spectacular auction event! 🎨✨ Thank you to everyone who participated and made it a success.`, RecipientType.USER);
